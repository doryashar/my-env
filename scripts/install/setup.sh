...
#   curl -fsSL https://raw.githubusercontent.com/doryashar/my-env/master/scripts/install/setup.sh | bash
#   # Or with custom repo:
#   curl -fsSL https://raw.githubusercontent.com/doryashar/my-env/master/scripts/install/setup.sh | ENV_URL=https://github.com/user/repo bash
#
# Features:
...
if [[ "${NEEDS_CLONE:-0}" == "1" ]]; then
    ...
    exec bash "$ENV_TARGET/scripts/install/setup.sh" "$@"
fi
...
create_private_repo() {
    ...
    echo "4. Click 'Create repository'"
        echo "5. Run: $ENV_DIR/scripts/sync/sync_encrypted.sh"
    echo ""
    return 1
}
...
sync_encrypted_files() {
    ...
    if check_remote_repo_exists "$private_url"; then
        ...
        bash "$ENV_DIR/scripts/sync/sync_encrypted.sh" || \
        ...
        if prompt_yn "Create repository? (y/n) "; then
            if create_private_repo "$private_url"; then
                bash "$ENV_DIR/scripts/sync/sync_encrypted.sh" || warning "Encrypted sync failed (non-fatal)"
            fi
        else
            ...
            echo "  3. Run: $ENV_DIR/scripts/sync/sync_encrypted.sh"
        fi
    fi
}
...
install_docker() {
    ...
    if [[ -f "$ENV_DIR/scripts/install/install_docker.sh" ]]; then
        bash "$ENV_DIR/scripts/install/install_docker.sh" || \
...
install_dotfiles() {
    ...
    if [[ -f "$ENV_DIR/scripts/sync/sync_dotfiles.sh" ]]; then
        bash "$ENV_DIR/scripts/sync/sync_dotfiles.sh" \
...
install_fonts() {
    ...
    if [[ -f "$ENV_DIR/scripts/install/install_fonts.sh" ]]; then
        bash "$ENV_DIR/scripts/install/install_fonts.sh" || warning "Font installation failed (non-fatal)"
    else
...
main() {
    ...
    echo "  3. Run: $ENV_DIR/scripts/sync/sync_env.sh --sync to sync your files"
}
