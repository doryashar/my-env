...
show_help() {
    ...
    echo "  -s, --sync                Sync dotfiles between local and repo"
...
handle_conflict() {
    ...
    case "$strategy" in
        "ask")
            ...
            case "$choice" in
                1) cp -f "$target" "$source" ;;
                2) cp -f "$source" "$target" ;;
                3) 
...
if [ -f "$script_path" ] && [ "\$-" = *i* ]; then
    ...
    # Uncomment to auto-sync on shell startup:
    # "$script_path" --sync
    alias dotsync="$script_path"
fi
EOL
        
        echo -e "${GREEN}Added to .bashrc. You can now use 'dotsync' command.${NC}"
    else
...
main() {
    ...
    if [[ -z "$PERFORM_INIT" && -z "$PERFORM_PULL" && -z "$PERFORM_SYNC" && -z "$PERFORM_PUSH" ]]; then
        show_help
    fi
    ...
    # Perform actions
    [[ -n "$PERFORM_PULL" ]] && git_sync "pull"
    [[ -n "$PERFORM_SYNC" ]] && sync_dotfiles
    [[ -n "$PERFORM_PUSH" ]] && git_sync "push"
